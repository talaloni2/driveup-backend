...
async def test_create_login_get_update_delete(test_client, unauthenticated, ensure_db_schema: None):
    ...
    response = await test_client.get(
        ...
        resp_model=GetUserByEmailResponse,
        headers={"Authorization": f"Bearer {token}"}
    )
    assert response.result == GetUserByEmailResult(full_name=new_username, email=new_username, car_color=None, phone_number=new_username, car_model=None, plate_number=None)

...
async def test_create_get_delete_user_subscription_map(test_client, test_user, ensure_db_schema: None):
    assert (
        ...
        code=200, status="OK", message="UserSubscriptionMap created successfully", result=None, detail=None
    )
    ...
    assert response == SubscriptionHandlerResponse(
        code=200,
        status="OK",
    ...
    assert (
        ...
        code=200, status="OK", message="UserSubscriptionMap deleted successfully", result=None, detail=None
    )
